...
export default function AllListing(props) {
    ...
    axios
      ...
      .then(async response => {
        ...
        if (response.data.status == 200) {
          setAllbooks(response.data.successData.books);
        }
      })
      .catch(error => {
        // handle error
  ...
  const GetBooks = async () => {
    ///// This Function is written to get All Books Listing through Api.
    setindicator(true);
    let token = await AsyncStorage.getItem('Auth');
    const AuthStr = 'Bearer '.concat(token);
    ...
    axios
      .post(`${url}/books`, formdata, {headers: {Authorization: AuthStr}})
      .then(async response => {
        console.log(response);
        if (response.data.status == 200) {
          setAllbooks(response.data.successData.books);
    ...
    <View style={styles.container}>
      {indicator ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <AdBanner />
          <Text style={styles.text}>All Listings</Text>
